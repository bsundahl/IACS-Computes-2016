...
for line in fin.readlines():
    ...
    if line_length==None:
        line_length = len(els)
    if len(els)==line_length:
        data_line = []
        for el in els:
            intensity = float(el)
            if intensity>scaling_max:
                intensity = 1.0
            elif intensity<scaling_min:
                intensity = 0.0
            else:
                intensity = (intensity-scaling_min)/scaling_span
            data_line.append( intensity )
